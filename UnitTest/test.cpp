...
TEST(MockTest_AddCustomer_OK, TestLibrary)
{
	...
	//auto myMockCustomerRepo = dynamic_cast<MockCustomerRepository*>(customerRepositoryManager.getCurrentCustomerRepository());
	//EXPECT_CALL(*myMockCustomerRepo, addCustomer).WillOnce(testing::Return(MyAssecorLibrary::AddResult::eDuplicatedID));
	//ON_CALL(*currentCustomerRepo, addCustomer). WillByDefault(testing::Return(MyAssecorLibrary::AddResult::eDuplicatedID));
}
